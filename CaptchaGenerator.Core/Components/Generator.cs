...
namespace CaptchaGenerator.Core
{
    public class Generator
    {
        const string allowedChars = "2346789ABCDEFGHJKLMNPRTUVWXYZ";
        const int length = 4;

        public static string GenerateCaptchaCode()
        {
            Random rand = new Random();
            int maxRand = allowedChars.Length - 1;
        ...
        public static bool ValidateCaptchaCode(string captchaCode, HttpContext context)
        {
            var isValid = captchaCode == context.Session.GetString("CaptchaCode");

            context.Session.SetString("CaptchaCode", "");
        ...
        public static CaptchaResult GenerateCaptchaImage(int width, int height, string captchaCode)
        {
            ...
            using (Graphics graph = Graphics.FromImage(baseMap))
            {
                ...
                return new CaptchaResult { code = captchaCode, byteData = ms.ToArray(), timestamp = DateTime.Now };
